...
%Mesh definition
%Define nodes
MESHstruct.COORDS = [0 1
                     1 1
                     1 0];


%Define truss elements through their connectivities
MESHstruct.IEN = [1 2
                  2 3];
...
% material properties
E   = 1;                         % Young modulus 
nee = 0.0;                       % poisson ratio
A   = 1;                         % cross section area
sy  = 1;                         % yield stress
h   = 0.1;                       % hardening modulus
...
%Set of nodes to which BCs are applied (nodes 1 and 3)
ind = [1 3];
...
%Set of nodes to which nodal loads are applied (node 2)
ind = 2;
...
% SOLstruct.arc_length = 0.1*ones(11,1); %Values of the 
%load factor for load control (simple loading)
SOLstruct.arc_length = [0.1*ones(11,1); -0.1*ones(24,1); 0.1*ones(12,1)]; %Values of the 
%load factor for load control (loading, unloading, reloading)

%Second case: displacement control
%The y displacement of the top right point of the truss
% SOLstruct.constraint = @(u, l, u0, l0, dup, dlp, si) displ_control(u, l, u0, l0, dup, dlp, si, 3);
% SOLstruct.predictor = 0; %Flag for using a predictor
% SOLstruct.arc_length = [0.1*ones(15,1); -0.1*ones(15,1)]; %Values of the
% %controlled displacement

%Third case: Riks method
% SOLstruct.constraint = @Riks;
% SOLstruct.predictor = 1; %Flag for using a predictor
% SOLstruct.arc_length = 0.1*ones(15,1); %Values of the arc length
