function  F_int = computeintForce(obj, u_elem)
% Computes Tangent Stiffness (KT) and Internal Force (F_int)
%ONLY an internal force is calculated for line-search procedure
...
for i = 1:Gpoint
    for j = 1:Gpoint
        ...
        for n = 1:8
            ...
            Gw(1, idx(1:3)) = dN_dx * theta(3,:); % d(w_loc)/dx
            Gw(2, idx(1:3)) = dN_dy * theta(3,:); % d(w_loc)/dy
            Gu(1, idx(1:3)) = dN_dx * theta(1,:); % d(u_loc)/dx
            Gu(2, idx(1:3)) = dN_dy * theta(1,:); % d(u_loc)/dy
            Gv(1, idx(1:3)) = dN_dx * theta(2,:); % d(v_loc)/dx
            Gv(2, idx(1:3)) = dN_dy * theta(2,:); % d(v_loc)/dy
        end

        % --- 3. Calculate Current Strains & Stresses ---
        % Displacement gradient vector theta_kw = G * u
        theta_kw = Gw * u_elem;
        theta_ku = Gu * u_elem;
        theta_kv = Gv * u_elem;
