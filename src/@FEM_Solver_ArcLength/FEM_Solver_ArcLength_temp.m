classdef FEM_Solver_ArcLength < FEM_Solver_Adaptive
    ...
    % Crisfield arc-length constraint (spherical constraint)
    % g = dup'*(u - u1) + dlp*(l - l1) = 0
    % where u1 = u0 + dlp*dup, l1 = l0 + dlp
    
    u1 = u0 + dlp * dup;
    l1 = l0 + dlp;
    
    g = dup' * (u - u1) + dlp * (l - l1);
    h = dup;        % gradient w.r.t. displacements
    s = dlp;        % gradient w.r.t. load factor
end

function [g, h, s] = loadControlConstraint(obj, u, l, u0, l0, dup, dlp, arc_length)
    % Load control constraint: l = l0 + arc_length
    % Simplifies to pure load stepping when arc_length represents load increment
    
    l_target = l0 + arc_length;
    g = l - l_target;
    
    h = zeros(length(u), 1);  % no displacement dependence
    s = 1;                    % gradient w.r.t. load factor
