...
%
% Automatically switches between serial and parallel (parfor) assembly 
% based on mesh size. Utilizes sparse triplet indexing for speed.
%
...
if numElems<5e6
    ...
    tic
    nz_per_elem = 48*48;
    estnz = int32(nz_per_elem * numElems);
    I_idx = zeros(estnz, 1);
    J_idx = zeros(estnz, 1);
    V_val = zeros(estnz, 1);
    count = 0;
