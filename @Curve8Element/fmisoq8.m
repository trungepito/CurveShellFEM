...
%
der(1,1)=0.25*etam*(2.*xi + eta);  der(1,5)=-1.*etam*xi;
der(1,2)=0.25*etam*(2.*xi-eta);  der(1,6)=0.5*(1-eta^2);
der(1,3)=0.25*etap*(2.*xi+eta);  der(1,7)=-1.*etap*xi;
der(1,4)=0.25*etap*(2.*xi-eta);  der(1,8)=-0.5*(1.-eta^2);
%
der(2,1)=0.25*xim*(2.*eta+xi);  der(2,5)=-0.5*(1. - xi^2);
der(2,2)=-0.25*xip*(xi-2.*eta);  der(2,6)=-1.*xip*eta;
der(2,3)=0.25*xip*(xi+2.*eta);  der(2,7)=0.5*(1.-xi^2);
der(2,4)=-0.25*xim*(xi-2.*eta);  der(2,8)=-1.*xim*eta;
%
% end function fmquad
