...
% Solves the system by enforcing a displacement at a specific node
% and calculating the required Reaction Force.
...
while runflag
    ...
    for iter = 1:maxIter
        ...
        [KT, F_int] = obj.assembleTangentSystem();

        % B. Calculate Residual
        % In pure displacement control without other external loads:
        % R = -F_int (We want F_int to go to 0 on free nodes)
        % If there are constant dead loads elsewhere, add them here.
        % R = F_ext_constant - F_int;
        R = -F_int;
        ...
        if res_norm < tol
            % CONVERGED
            % Calculate Reaction Force at Control Node
            % Reaction = Internal Force at that node (since Ext is 0)
            % or Reaction = - Residual (if Ext exists)
            % reaction_force = norm(F_int(c_dof));
