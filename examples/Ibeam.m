% EXAMPLE_Complex_Beam.m
clear; clc; close all

% 1. Init
Pre = FEM_Preprocessor_v2(2e9, 0.3, 0.05);

% 2. Geometry: Generate Thin-Walled I-Beam
% Flange Width=0.2m, Height=0.3m, Length=2m
% The 'createIBeam' macro internally generates 3 patches
nseg=6;
Lseg=0.5; %m
L=Lseg*nseg;
Pre.createIBeam(0.300, 0.200, Lseg,nseg,[5,10]);

% 3. Meshing
% Mesh all patches with 4 elements width, 20 elements length
% Pre.meshAllPatches(8, 20);

% 4. Selection & BCs
% Fix the Root (Z=0)
% Using 'plane' selector: Dim 3 (Z) is 0.0
web0=Pre.selectNodesByBox(-0.01,0.01,-0.01,0.01,-0.150,0.150);
cros0=Pre.selectNodesOnPlane(1,0,1e-5);
flange0=setdiff(cros0,web0);

webL=Pre.selectNodesByBox(L-0.01,L+0.01,-0.01,0.01,-0.150,0.150);
crosL=Pre.selectNodesOnPlane(1,L,1e-5);
flangeL=setdiff(crosL,webL);

% Select Top Flange Elements for Distributed Load
% Top Flange is at Y > 0.
% Box Selection: X(-0.2 to 0.2), Y(0.1 to 0.2), Z(0 to 2)
% topFlangeElems = Pre.selectNodesOnPlane(1,3*2000,1e-5);
webm=Pre.selectNodesByBox(nseg/2*Lseg-0.01,nseg/2*Lseg+0.01,-0.01,0.01,-0.150,0.150);
midspan = Pre.selectNodesOnPlane(1,nseg/2*Lseg,1e-5);

%fixed at 0
Pre.addBC(cros0,1:6,0,'Support');
%pinned at L
Pre.addBC(webL,2,0,'Support');
Pre.addBC(flangeL,3,0,'Support');
%%
% 6.2 Nonlinear GNI analysis
%Disp load
Pre.addBC(webm,2,0.05,'Disp1');
Pre.addBC(crosL,1,-0.05,'Disp2');

S1 = LoadingStage(1.0);
S1.activateBC('Support');
S1.activateBC('Disp1');
% S1.activateLoad('Load1');
S2=LoadingStage(1.0);
S2.activateBC('Support');
S2.activateBC('Disp1');
S2.activateBC('Disp2');
% S2.activateBC('Undisp')
% 
% S3=LoadingStage(1.0);
% S3.activateBC('Support');
% S3.activateLoad('Load1');

solOpt=SolverOptions;
Sol=FEM_Solver_Adaptive(Pre,solOpt);
Sol.solve({S1});
%%
% 7. Post
Post = FEM_Postprocessor(Pre, Sol);
opts.layer='Top';
opts.scale=5;
opts.Nummode=1;
Post.plotField('sY', opts);
title('I-Beam Bending under Pressure');

% for ii=1:10
%     opts.Nummode=ii;
% Post.plotField('Buckling', opts);
% title('Buckli0ng mode 1 (Bot)');
% colormap jet;
% end
colormap jet;
%% 
% 5. Post-Process
Post = FEM_Postprocessor(Pre, Sol);

% A. Plot Curve
% Plot Displacement of a tip node (e.g., center of tip)
% midTip = tipNodes(round(end/2));
% Post.plotLoadDisplacement(web0(1), 3); % Z-disp
opts.layer='Top';
opts.scale=10;
opts.Nummode=1;
Post.plotField('Displacement', opts);
title('I-Beam Bending under Pressure');

% Post.animateDisplacement(10,5,0)
% for i=1:size(Sol.U_Hist,2)
%     Sol.U=Sol.U_Hist(:,i);
% Post.plotField('Displacement', opts);
% title('Displacement Magnitude NL');
% end
% % B. Animate
% % Scale = 1.0 %(True scale to see real rotation)
% Post.animateScenario(crosL(1), 1, 1, 0.5);